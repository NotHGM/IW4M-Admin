...
const pollingRate = 1000; // todo: adjust when iw4x updated
...
const plugin = {
    ...
    onServerValueSetCompleted: function(serverValueEvent) {
        ...
        if (serverState.inQueue.length > 0 && serverValueEvent.valueName === inDvar) {
            ...
            if (this.processEventMessage(input, serverValueEvent.server)) {
                // return;
            }
        }
                            ...
                            if (event.data['direction'] != null) {
                                ...
                                if (!isNaN(parsedValue)){
                                    event.data['direction'] = 'up' ?
                                        metaService.incrementPersistentMeta(key, parsedValue, clientId, token).getAwaiter().getResult() :
                                        metaService.decrementPersistentMeta(key, parsedValue, clientId, token).getAwaiter().getResult();
                                }
                            } else {
...
const getClientStats = (client, server) => {
    ...
    const stats = context.clientStatistics.getClientsStatData([client.ClientId], server.getId());
    context.dispose();
